...
面向当前 ComfyUI 原生 MiniMax H3 的独立 T8 节点扩展。当前版本为 `1.35.1`，共注册
...
`sampling.py`保持不变。
`1.35.1`继续保留5个学习型Latent Advanced节点，并把默认I2VA工作流校正到上游提交`64fc9d4`：
倍率模式默认2.0×，放大节点输出的实际32对齐宽高直接连接第二套Conditioning，用户不再维护容易错位的
第二份尺寸。低清736×416现在得到1472×832，而不是旧示例的1120×640。采样使用底模原生shift 12/3、
`simple8`前4步及原始高分3步sigma`0.9035, 0.6316, 0.3158, 0`。量化底模改用
`LoraLoaderBypassModelOnly`加载校验正确的`comfyui_alpha8` LightX2V LoRA；已否决的普通`_comfyui`
转换缺少PEFT alpha处理，会产生约16倍LoRA更新并造成整幅融化。真实1472×832×124联合AV链完成且
严格解码通过，接触表未见前一失败样本的整幅崩坏。旧已保存工作流的显式参数、节点ID/输入顺序可继续
加载；原普通插值`Latent Upscale by 32`、前128个节点和稳定采样数学均未改动。
...
`187eda8ef5e5`；本轮真实学习型二采由当前一键包启动入口`0f1fa67ad8a6`执行，相关74项回归通过；上一轮Qwen缓存/H3
...
`1.35.1`的真实I2VA探针使用FL2VA full INT8、corrected alpha8 LightX2V LoRA+bypass、同一首帧/提示词/seed，
在736×416×124执行低4步，以学习型3D模型按2.0×放大到1472×832；放大节点的实际宽高直接驱动
高分Conditioning，随后以shift 12/3执行原始高3步。7次联合AV前向在626.969秒内完成；峰值整卡使用
约15,170.3MiB。最终H.265为1472×832、124帧、24fps、32kHz双声道，A/V时差14.67ms，视频和音频
分别通过`-xerror -err_detect explode`严格完整解码，SHA-256为
`06BEE97DAC218956FBB83288397878481211A6BA626EEA599667C897500924E0`。8帧接触表未出现错误LoRA路线的
整幅融化；这证明尺寸联动、正确LoRA和媒体链可用，不代表跨素材必然优于全分辨率基线。
上游/本地网络在同权重、同随机输入上的322个参数和输出均逐位相同。本机VHS H.264路线曾出现一帧损坏，
示例继续固定H.265。
...
为兼容旧工作流和提示词生成器，普通自然语言中的 `Video 2` / `Audio 3` 在没有连接该类参考媒体时
保留为普通文本并写入 report，不再被误判成致命标签；`<Picture 0>`等旧式0起始编号会映射到
官方1起始编号。若同类媒体只连接一份，遗留的其他正整数编号会唯一映射到第1份并报告修正。
已经连接多份同类媒体却引用越界编号，或显式`<Picture N>` / `<Video N>` / `<Audio N>`完全没有
对应媒体时，严格模式仍会停止，防止参考对象静默错绑。关闭严格模式时，无法连接的显式标签会降级为
普通文本，而不是把悬空媒体标签交给Qwen。
